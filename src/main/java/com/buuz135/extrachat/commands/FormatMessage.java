...
public class FormatMessage implements CommandExecutor {
    @Override
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        if (args.<String>getOne("format").isPresent()) {
            if (!args.<String>getOne("format").get().contains("%PLAYER%") || !args.<String>getOne("format").get().contains("%MES%")) {
            ...
            ConfigLoader.saveConfig(args.<String>getOne("format").get(), "formatMes");
            src.sendMessage(Texts.of(TextColors.BLUE, "[", TextColors.DARK_AQUA, "EC", TextColors.BLUE, "] ",
